...
void sfml_window::MainMenu::LoadButtons() {
  ...
  buttons_[(unsigned)MainMenuButton::EXIT] =
      new TextButton({{window_width_ - 100, 0}, 100, 100}, std::string("EXIT"));
  ...
  buttons_[(unsigned)MainMenuButton::PLAY_LEVEL] = new TextButton(
      AlignCenter({{0, 100}, 400, 100}), std::string("Play level"));
...
Rect sfml_window::MainMenu::AlignCenter(Rect default_position) {

  default_position.placement.x =
      (window_width_ / 2) - default_position.width / 2;
